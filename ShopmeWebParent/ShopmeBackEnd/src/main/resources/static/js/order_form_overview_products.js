...
function setAndFormatNumberForField(fieldId, fieldValue) {
	formattedValue = $.number(fieldValue, decimalDigits, decimalSeparator, thousandsSeparator);
...
function formatNumberForField(fieldRef) {
	fieldRef.val($.number(fieldRef.val(), decimalDigits, decimalSeparator, thousandsSeparator));
}
...
function removeThousandSeparatorForField(fieldRef) {
	fieldRef.val(fieldRef.val().replace(thousandsSeparator, ""));
	fieldRef.val(fieldRef.val().replace(decimalSeparator, "."));
}
